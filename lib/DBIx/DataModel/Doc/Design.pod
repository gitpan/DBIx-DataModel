...
L<DBIx::DataModel::Doc::Delta_2.0>). The documentation below has
B<not been fully updated yet> to reflect these changes.
...
or
L<Join|DBIx::DataModel::Source::Join> subclass,
L<join()|DBIx::DataModel::Doc::Reference/Table::join()> 
is a class method that creates a statement to
query one or several associated tables.
The statement contains a condition to restrict the results
according to the primary key of the initial table.
That statement cannot be executed yet, because the values of the primary
key are not known until we have an instance of the initial table; but the
...
L<DBIx::DataModel::Schema|DBIx::DataModel::Schema>).
While in this state, the statement can
accumulate parameters (C<-columns> to retrieve, C<-where> clauses
that will end up in the generated SQL, etc.) through one or 
several calls to the 
L<refine()|DBIx::DataModel::Doc::Reference/refine()>
method.  Early parameter binding may also occur through the
...
The philosophy of C<DBIx::DataModel> is that a record is nothing more
than a blessed hashref, where hash keys are column names and hash
values are column values.  So data access uses goes through the
hashref API : this allows you to exploit all common Perl idioms, like
