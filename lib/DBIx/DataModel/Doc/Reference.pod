...
If C<< $pckName >> contains no C<::>, then the schema name
...
in the C<< MySchema->Table(...) >> declarations. However, tables may 
...
on the primary key(s) of C<My::Table>.  That statement cannot
be executed yet, because the values of the primary key are not
known until we have an instance of C<My::Table>; but the statement
can already be
...
to access data when using C<DBIx::DataModel>.
...
Returns an arrayref containing C<< [\"($sql)", @bind] >>. 
This is in anticipation for the future version of 
L<SQL::Abstract|SQL::Abstract> (coming soon) where we will be able
...
as a list of values. Note that C<< My::Table->delete(11, 22) >>
